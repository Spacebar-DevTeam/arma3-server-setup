...
// Control Existing Random Spawns
{
	...
	else
	{
		...
		_distanceFromSelectedPlayer = 1200;
		...
		if(!alive _selectedPlayer OR _distanceFromSelectedPlayer >= 750) then
		{
			// Select a new target or despawn if no target nearby
			_nearPlayers = _groupLeader nearEntities ["Exile_Unit_Player", 1000];	// Increased search radius
			{
				_selectedPlayer = _x;
			...
			if(count(_suitableTargets) > 0) then
			{
				...
				{
					_group reveal [_selectedPlayer,1.0]; // Reduced to avoid instant sniping
					
					_group allowFleeing 0;
					_wp = _group addWaypoint [_destination, 0] ;
					_wp setWaypointFormation "Column";
					_wp setWaypointBehaviour "SAFE"; // Start with a relaxed behavior
					_wp setWaypointCombatMode "WHITE"; // Avoid immediate aggression
					_wp setWaypointCompletionRadius 15; // Give a bit of a buffer
