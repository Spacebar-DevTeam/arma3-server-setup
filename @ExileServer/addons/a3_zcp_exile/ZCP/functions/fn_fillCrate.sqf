/*
	...
	The fn_fillCrate.sqf script fills a loot crate for ZCP missions by randomly selecting 
	weapons, items, and backpacks from configured arrays or custom loot sets. It supports 
	standard loot, custom loot sets, and rare loot with configurable drop chances. 
	The script includes debug logging for loot distribution details.
	
	Improvements Made:
      Debug Variable: Added ZCP_Debug = false; at the top.
      Simplified Logic: Merged similar operations into reusable functions.
      Debugging: Enhanced logging for each loot type added.
      Safety Checks: Ensures valid loot items before adding them to the crate.
      Performance: Utilizes params and private for efficient variable handling.
...
params [
    "_crate",
    "_lootValues",
    ["_rareLootChance", ZCP_DMS_RareLootChance, [0]]
];

// Validate input parameters
if (isNull _crate || {!(typeName _lootValues in ["ARRAY", "STRING"])}) exitWith {
    diag_log format ["[ZCP] ERROR: Invalid parameters for ZCP_fnc_fillCrate: %1", _this];
};

// Unhide crate
_crate hideObjectGlobal false;

// Initialize loot arrays
private _wepCount = 0;
private _weps = [];
...
if (typeName _lootValues == "ARRAY" && !(typeName (_lootValues select 1) == "CODE")) then {
    // Weapons
    [_wepCount, _weps] = if (typeName (_lootValues select 0) == "ARRAY") then {
        _lootValues select 0
    } else {
        [_lootValues select 0, ZCP_DMS_boxWeapons]
    };

    // Items
    [_itemCount, _items] = if (typeName (_lootValues select 1) == "ARRAY") then {
        _lootValues select 1
    } else {
        [_lootValues select 1, ZCP_DMS_boxItems]
    };

    // Backpacks
    [_backpackCount, _backpacks] = if (typeName (_lootValues select 2) == "ARRAY") then {
        _lootValues select 2
    } else {
        [_lootValues select 2, ZCP_DMS_boxBackpacks]
    };
};

// Add loot to the crate
private _addToCrate = {
    params ["_crate", "_count", "_lootArray", "_addFunc"];
    for "_i" from 1 to _count do {
        private _item = _lootArray call BIS_fnc_selectRandom;
        if (!isNil "_item") then {
            [_crate, _item] call _addFunc;
            if (ZCP_Debug) then {
                diag_log format ["[ZCP] Added item to crate: %1", _item];
            };
        };
    };
};
...
[_crate, _wepCount, _weps, {
    params ["_crate", "_weapon"];
    _crate addWeaponCargoGlobal [_weapon, 1];
    private _ammo = _weapon call ZCP_fnc_selectMagazine;
...
// Add items
[_crate, _itemCount, _items, {
    params ["_crate", "_item"];
    _crate addItemCargoGlobal [_item, 1];
}] call _addToCrate;

// Add backpacks
[_crate, _backpackCount, _backpacks, {
    params ["_crate", "_backpack"];
    _crate addBackpackCargoGlobal [_backpack, 1];
}] call _addToCrate;

// Add rare loot if applicable
if (ZCP_DMS_RareLoot && {count ZCP_DMS_RareLootList > 0} && {random 100 < _rareLootChance}) then {
    private _rareItem = ZCP_DMS_RareLootList call BIS_fnc_selectRandom;
    _crate addItemCargoGlobal [_rareItem, 1];
    if (ZCP_Debug) then {
        diag_log format ["[ZCP] Added rare loot to crate: %1", _rareItem];
    };
};

if (ZCP_Debug) then {
    diag_log format ["[ZCP] Crate filled at %1 with weapons, items, and backpacks.", getPosATL _crate];
};
