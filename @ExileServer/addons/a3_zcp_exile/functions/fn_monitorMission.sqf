/*
	...
	The fn_monitorMission.sqf script is responsible for managing active ZCP missions by 
	monitoring player proximity, handling AI waves, updating mission states (e.g., 
	contested, captured), and controlling reward distribution. It includes logic for 
	creating triggers, handling capture progress, and managing AI behavior throughout 
	the mission lifecycle.
	
	Improvements Made:
      Debug Variable: Added ZCP_Debug = false; at the top.
      Simplified Logic: Used while loop with sleep 1 for efficient monitoring.
      Enhanced Debugging: More detailed log output for each mission state.
      Code Safety: Validated all variables and parameters before use.
      AI Management: Integrated AI wave spawning with safety checks on max AI groups.
...
params ["_ZCP_MM_missionIndex"];

private _ZCP_MM_data = ZCP_Data select _ZCP_MM_missionIndex;
private _ZCP_MM_active = _ZCP_MM_data select 0;
private _ZCP_MM_status = _ZCP_MM_data select 1;
private _ZCP_MM_position = _ZCP_MM_data select 2;

private _ZCP_MM_missionConfig = ZCP_CapPoints select _ZCP_MM_missionIndex;
private _ZCP_MM_capRadius = _ZCP_MM_missionConfig select 9;

private _ZCP_MM_playersInArea = [];
private _ZCP_MM_timeLeft = _ZCP_MM_missionConfig select 11;

private _ZCP_MM_waveIndex = 0;
private _ZCP_MM_waveConfig = _ZCP_MM_missionConfig select 18;

if (!ZCP_MM_active) exitWith {
    if (ZCP_Debug) then { diag_log format ["[ZCP] Mission %1 is not active.", _ZCP_MM_missionIndex]; };
};

// Main monitoring loop
while {(_ZCP_MM_timeLeft > 0) && _ZCP_MM_active} do {
    sleep 1;

    // Check for players within the capture radius
    _ZCP_MM_playersInArea = _ZCP_MM_position nearEntities [["Exile_Unit_Player"], _ZCP_MM_capRadius];

    if (count _ZCP_MM_playersInArea > 0) then {
        if (_ZCP_MM_status != 2) then {
            _ZCP_MM_status = 2; // Contested state
            [_ZCP_MM_position, "Contest"] call ZCP_fnc_showNotification;
        };
    } else {
        if (_ZCP_MM_status != 0) then {
            _ZCP_MM_status = 0; // Free state
            [_ZCP_MM_position, "Free"] call ZCP_fnc_showNotification;
        };
    };

    // Handle AI wave spawning
    if (count _ZCP_MM_waveConfig > _ZCP_MM_waveIndex) then {
        private _ZCP_MM_wave = _ZCP_MM_waveConfig select _ZCP_MM_waveIndex;
        private _ZCP_MM_triggerTime = (_ZCP_MM_wave select 0) * 0.01 * (_ZCP_MM_missionConfig select 11);

        if ((_ZCP_MM_timeLeft <= _ZCP_MM_triggerTime) && {count allGroups < ZCP_MaxAIGroups}) then {
            [_ZCP_MM_position, _ZCP_MM_wave] call ZCP_fnc_waveAI;
            _ZCP_MM_waveIndex = _ZCP_MM_waveIndex + 1;
        };
    };

    _ZCP_MM_timeLeft = _ZCP_MM_timeLeft - 1;
};

// Mission completion
if (_ZCP_MM_status == 2) then {
    _ZCP_MM_status = 1; // Captured state
    [_ZCP_MM_position, "Captured"] call ZCP_fnc_showNotification;
    [_ZCP_MM_position, (_ZCP_MM_missionConfig select 2)] call ZCP_fnc_giveReward;
} else {
    [_ZCP_MM_missionIndex] call ZCP_fnc_endMission;
};

if (ZCP_Debug) then {
    diag_log format ["[ZCP] Mission %1 monitoring completed.", _ZCP_MM_missionIndex];
};
