/*
	...
	The fn_createDMSSoldier.sqf script is responsible for spawning individual AI 
	units for ZCP missions, configuring their difficulty, and equipping them 
	according to predefined parameters. 
	
	It handles AI skills, spawning at valid 
	positions, and setting up behavior such as disabling fleeing and certain AI 
	features for mission control.

	Improvements Made:
	  Error Handling: Added isNull check to prevent spawning issues.
	  Performance Optimization: Simplified loops and used forEach for configuration.
	  Debug Logging: Provides feedback on AI creation and skill settings.
	  Safety Checks: Avoids null references and ensures valid spawn positions.
	  Consistency: Maintains uniform coding standards with other ZCP functions.
...
private _ZCP_CDS_skill = switch (toLower _ZCP_CDS_difficulty) do {
    case "random": {DMS_ai_skill_random call BIS_fnc_selectRandom};
    case "randomdifficult": {DMS_ai_skill_randomDifficult call BIS_fnc_selectRandom};
    case "randomeasy": {DMS_ai_skill_randomEasy call BIS_fnc_selectRandom};
    case "randomintermediate": {DMS_ai_skill_randomIntermediate call BIS_fnc_selectRandom};
    default {_ZCP_CDS_difficulty};
};

// Find a valid spawn position
private _ZCP_CDS_posAI = _ZCP_CDS_spawnAIPos findEmptyPosition [0, _ZCP_CDS_radius, _ZCP_CDS_soldierClass];

// Spawn the AI unit
private _ZCP_CDS_unitAI = _ZCP_CDS_dummyGroup createUnit [_ZCP_CDS_soldierClass, _ZCP_CDS_posAI, [], 0, "FORM"];
if (isNull _ZCP_CDS_unitAI) exitWith {
    diag_log "[ZCP] ERROR: Failed to create AI unit.";
};
...
} forEach ["AUTOTARGET", "TARGET", "MOVE"];

// Clear existing gear
removeAllWeapons _ZCP_CDS_unitAI;
removeAllItems _ZCP_CDS_unitAI;
removeAllAssignedItems _ZCP_CDS_unitAI;
removeAllContainers _ZCP_CDS_unitAI;

// Apply difficulty-based skill settings
{
    _ZCP_CDS_unitAI setSkill [_x, _ZCP_CDS_skill];
} forEach ["aimingAccuracy", "aimingShake", "aimingSpeed", "spotDistance", "spotTime", "courage", "reloadSpeed", "commanding", "general"];

// Debugging log
if (ZCP_Debug) then {
    diag_log format ["[ZCP] Created AI unit: %1 with skill %2", _ZCP_CDS_unitAI, _ZCP_CDS_skill];
};
