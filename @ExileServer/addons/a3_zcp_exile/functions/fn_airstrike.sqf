/*
    ...
    Error Handling: Avoids null objects and out-of-bounds positions.
    Optimized Waypoints: Uses Seek and Destroy (SAD) to focus attacks.
    Improved Bombing Logic: Targets all valid objects in the base area.
    Debug Mode: Controlled by ZCP_Debug for logging and hints.
    Automatic Cleanup: Deletes the plane after 120 seconds to prevent lingering objects.
...
private _baseObjects = _this select 0; 		// Base objects to target
private _targetPos = _this select 1; 		// Target position for the strike
private _baseRadius = _this select 2; 		// Effective radius around the base
private _originPos = [random (ZCP_MapRadius * 2), random (ZCP_MapRadius * 2), 500]; // Randomized spawn position
private _planeType = "B_Plane_CAS_01_F"; 	// Default plane type for airstrike
private _plane; 							// Variable for the plane object

// Adjust spawn position to avoid edge cases
_originPos set [0, if (round (random 1) > 0) then {10} else {_originPos select 0}];
_originPos set [1, if (round (random 1) > 0) then {10} else {_originPos select 1}];
_targetPos set [2, ZCP_FlyHeight]; 			// Maintain specified flight height

// Spawn the plane and set up the pilot
_plane = createVehicle [_planeType, _originPos, [], 0, "FLY"];
private _pilot = createVehicleCrew _plane select 0;

// Set plane attributes
_plane setVehicleVarName "ZCP_Airstrike_Plane";
_plane setDir ([_originPos, _targetPos] call BIS_fnc_dirTo);
_plane allowDamage false;

// Create waypoints for the strike
...
_wp2 setWaypointType "SAD"; 				// Seek and Destroy
...
if (count _baseObjects > 0) then {
    {
        if (!isNull _x) then {
            _plane doTarget _x;
            _plane fireAtTarget [_x, "Bomb_04_Plane_CAS_01_F"];
        };
...
// Debugging
if (ZCP_Debug) then {
    diag_log format ["[ZCP] Airstrike initiated at %1 with plane %2", _targetPos, _planeType];
    hint "Airstrike executed!";
};
